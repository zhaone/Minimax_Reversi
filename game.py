...
class Game(object):
    ...
    def print_winner(self, winner):
        ...
        :return: None
        """
        print(['黑棋获胜(Black player wins)!', '白棋获胜(White player wins)!', '平局(Draw)'][winner])
    ...
    def run(self):
            ...
            try:
                for i in range(0, 3):
                    ...
                    if action not in legal_actions:
                        ...
                        print("你落子不符合规则,请重新落子！(Invalid location, Please relocate!)")
                        continue
                    else:
                        # 落子合法则直接 break
                        break
                    ...
                    print("落子3次不合法，结束游戏！(3 consective invalid locations, game ends!)")
                    break
            except FunctionTimedOut:
                # 落子超时，结束游戏
                winner, diff = self.force_loss(is_timeout=True)
                print("操作超时，游戏结束！(The operation times out, the game ends!)")
                break
        ...
        if winner is not None and diff > -1:
            result = {0: '黑方胜(black_win)', 1: '白方胜(white_win)', 2: '平局(draw)'}[winner]
